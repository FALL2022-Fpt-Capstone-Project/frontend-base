...
const RoomContract = () => {
    ...
    const apartmentGroup = async () => {
        ...
        await axios
            ...
            .then((res) => {
                const data = res.data.data.list_group_contracted.find((obj, index) => obj.group_id === parseInt(state?.group_id)).list_room_lease_contracted;
                setDataApartmentGroup(res.data.data.list_group_contracted);
                setDataApartmentGroupSelect(data[0]);
                setRoom(
                    data[0]?.list_room?.filter(
                        (data) => data.room_floor === parseInt(state?.room_floor) && data.contract_id === null
                    )
                );
                setDataAsset(
                    data[0]?.list_hand_over_assets?.filter((asset => asset.hand_over_asset_quantity > 0))?.map(
                        (o, i) =>
                            [
                                {
                                    asset_id: o.asset_id,
                ...
                setFloorRoom((pre) => {
                    return { ...pre, room_floor: state?.room_floor };
                });
                setRoomSelect(data[0]?.list_room?.find((obj) => obj.id === state?.room_id));
                form.setFieldsValue({
                    group_id: state?.group_id,
                    room_floor: state?.room_floor,
                    room_id: state?.room_id,
                    contract_price: state?.room_price,
                    contract_deposit: state?.room_price,
                });
            })
            .catch((error) => {
                console.log(error);
    ...
    const onOk = () => {
        console.log(selectOldRenter.address.address_more_details);
    ...
    const addAssetFinish = (e) => {
        ...
        if (!duplicate) {
            setDataAsset([
                ...dataAsset,
                { ...e, hand_over_date_delivery: moment(e.hand_over_date_delivery).format("DD-MM-YYYY") },
            ]);
