...
function AddMemInRoom(data) {
    ...
    const getAllContract = async () => {
        ...
        await axios
            ...
            .then((res) => {
                setContractRoom(res.data.data?.find((obj, index) => obj.room_id === parseInt(room_id)));
            })
            .catch((error) => {
                console.log(error);
            });
            ...
            >
                ...
                <Layout className="site-layout">
                    <Header className="layout-header">
                        <p className="header-title">Quản lý thành viên trong phòng</p>
                    </Header>
                    <Content className="layout-content">
                        ...
                        >
                            ...
                            <Row>
                                <Col span={23}>
                                    <Form.Item className="form-item" name="list_renter" labelCol={{ span: 24 }}>
                                        <h3>
                                            <b>
                                                Thông tin về thành viên phòng {contractRoom?.room_name}
                                            </b>{" "}
                                            (Số lượng: {contractRoom?.list_renter?.length}/{contractRoom?.room?.room_limit_people})
                                        </h3>
                                    </Form.Item>
                                </Col>
                                <Col span={1}>
                            ...
                            <Modal
                                title={"Thêm thành viên vào Phòng " + contractRoom?.room_name}
                                open={isAddMem}
                                onOk={() => {
                                    setIsAddMem(false);
                                }}
